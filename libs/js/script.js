...
const API_BASE_URL = 'https://clockinapp.brightronix.net/backend';
...
async function pollUserTimer() {
    try {
        const data = await apiFetch('get_my_timer.php');
        if (data.success) {
...
function checkAuthAndRedirect() {
    ...
    if (!uid && !path.includes('/frontend/login/login.html')) {
...
async function registerAttendance(action) {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
        ...
        try {
            ...
            const response = await apiFetch('Registros.php', 'POST', payload);
            if (!response.success) {
                throw new Error(response.message || 'Could not register attendance.');
            }
...
async function generateQrCode() {
    ...
    try {
        const response = await apiFetch('GenerateQR.php', 'POST', { project_id: projectId });
        if (response.success) {
            ...
            if (qrMessage) qrMessage.textContent = `Error: ${response.message}`;
        }
    } catch (error) {
...
async function loadAttendanceRecords(uid = null) {
    ...
    const role = normalizeRole(sessionStorage.getItem('user_role'));
    const endpoint = role === 'admin' && !uid ? 'Registros.php?all=true' : `Registros.php?user_id=${uid || sessionStorage.getItem('user_id')}`;
...
async function loadActiveTimers() {
    ...
    try {
        const data = await apiFetch('get_active_timers.php');
        if (!data.success) {
            if (messageEl) {
                messageEl.textContent = data.message || 'Could not load timers.';
                messageEl.className = 'error-message';
            }
...
async function updateTimerStatus(timerId, status) {
    ...
    try {
        ...
        const response = await apiFetch('update_timer_status.php', 'POST', payload);
        if (!response.success) {
            throw new Error(response.message || 'Failed to update timer status.');
        }
...
async function loadSpecialUserRecords(userId) {
    ...
    try {
        const data = await apiFetch(`get_user_records.php?user_id=${userId}`);
        if (!data.success) {
            tbody.innerHTML = `<tr><td colspan="8">${data.message || 'Unable to load records.'}</td></tr>`;
            return;
        }
...
async function loadHoursChart(uid = null) {
    ...
    const role = normalizeRole(sessionStorage.getItem('user_role'));
    let endpoint = 'Registros.php?summary=true';
    if (role !== 'admin') {
...
async function loadUserLocationsAndDisplayOnMap() {
    ...
    try {
        const data = await apiFetch('GetUserLocations.php');
        if (data.success && data.locations.length > 0) {
            ...
            if (msg) msg.textContent = data.message || 'No recent user locations to display.';
        }
    } catch (error) {
        const msg = document.getElementById('map-message');
...
async function displayUserLocationHistory(uid, startDate, endDate) {
    ...
    try {
        const endpoint = `GetUserLocations.php?user_id=${uid}&start_date=${startDate}&end_date=${endDate}`;
        const data = await apiFetch(endpoint);
...
async function deleteRecord(recordId) {
    ...
    try {
        const response = await apiFetch('Registros.php', 'DELETE', { id: recordId });
...
async function loadUsers() {
    ...
    try {
        ...
        `;
            ...
            usersTableBody.innerHTML = `<tr><td colspan="4">Error loading users: ${data.message}</td></tr>`;
        }
    } catch (error) {
...
window.loadRolesIntoSelect = async function (selectId) {
    try {
        const data = await apiFetch('roles.php');
        if (data.success) {
...
async function saveUser(e) {
    ...
    const action = uidInput ? 'update' : 'create';
    const payload = { action, username: usernameInput, role_id: roleId };
    if (passwordInput) payload.password = passwordInput;
    if (uidInput) payload.id = uidInput;

    try {
        const response = await apiFetch('Users.php', 'POST', payload);
        if (messageEl) {
...
async function deleteUser(id) {
    ...
    const messageEl = document.getElementById('user-form-message');
    try {
        const response = await apiFetch('Users.php', 'POST', { action: 'delete', id });
        if (messageEl) {
            messageEl.textContent = response.message;
            messageEl.className = response.success ? 'success-message' : 'error-message';
        }
...
async function saveProject(e) {
    ...
    try {
        const response = await apiFetch('Projects.php', 'POST', payload);
        if (messageEl) {
            messageEl.textContent = response.message;
            messageEl.className = response.success ? 'success-message' : 'error-message';
        }
...
async function deleteProject(id) {
    ...
    const messageEl = document.getElementById('project-form-message');
    try {
        const response = await apiFetch('Projects.php', 'POST', { action: 'delete', id });
        if (messageEl) {
            messageEl.textContent = response.message;
            messageEl.className = response.success ? 'success-message' : 'error-message';
        }
...
async function deleteQr(qrId) {
    ...
    try {
        const response = await apiFetch('GeneratedQRs.php', 'DELETE', { id: qrId });
        alert(response.message);
...
async function loadGeneratedQRs() {
    ...
    try {
        const data = await apiFetch('GeneratedQRs.php');
        if (data.success) {
...
function startLocationTracking() {
    ...
    const path = window.location.pathname.toLowerCase();
    if (!path.includes('/frontend/registros/registros.html')) return;
    ...
    const logLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            try {
                await apiFetch('LogLocation.php', 'POST', {
                    user_id: uid,
                    latitude,
                    longitude
                });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginForm && path.includes('/frontend/login/login.html')) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await apiFetch('Login.php', 'POST', { user: usr, pass: pass });
                if (response.success) {
    ...
    if (recalcBtn) {
        recalcBtn.addEventListener('click', async () => {
            ...
            try {
                const response = await apiFetch('Admin.php', 'POST', { action: 'recalculate_daily_duration', user_id: uidSel, date });
                alert(response.message);
    ...
    if (editModal) {
        ...
        if (editForm) {
            editForm.addEventListener('submit', async (e) => {
                ...
                try {
                    await apiFetch('Registros.php', 'PUT', {
                        id: recordId,
